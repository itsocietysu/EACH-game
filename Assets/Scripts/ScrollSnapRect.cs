...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    public List<Sprite> unselectedPage;
    ...
    void Start() {
        ...
        if (footerFeedButton) 
            footerFeedButton.GetComponent<Button>().onClick.AddListener(() => { FeedScreen(); });
        if (footerMusButton)
    ...
    private void SetPageSelection(int aPageIndex) {
        ...
        if (_previousPageSelectionIndex >= 0) {
            _pageSelectionImages[_previousPageSelectionIndex].sprite = unselectedPage[_previousPageSelectionIndex];
            _pageSelectionImages[_previousPageSelectionIndex].SetNativeSize();
        }

        // select new
        _pageSelectionImages[aPageIndex].sprite = selectedPage[aPageIndex];
